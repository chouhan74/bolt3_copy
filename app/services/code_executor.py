...
class CodeExecutor:
    ...
    LANGUAGE_CONFIGS = {
        ...
        "java": {
            "extension": ".java",
            "compile_command": ["{javac_path}", "{filename}"],
            "run_command": ["{java_path}", "{classname}"],
            "timeout": settings.CODE_EXECUTION_TIMEOUT
        }
    }
    ...
    def __init__(self):
        self.temp_dir = tempfile.mkdtemp()
        self.java_paths = self._detect_java_paths()
        logger.info(f"Java paths detected: {self.java_paths}")
    
    def _detect_java_paths(self) -> Dict[str, str]:
        ...
        if not os.environ.get('JAVA_HOME'):
            from app.core.config import settings
            if hasattr(settings, 'JAVA_HOME') and os.path.exists(settings.JAVA_HOME):
        ...
        for java_path in java_locations:
            if os.path.exists(java_path) or shutil.which(java_path):
                paths['java'] = java_path
                logger.info(f"Found Java at: {java_path}")
                break
        
        # Check for javac
        for javac_path in javac_locations:
            if os.path.exists(javac_path) or shutil.which(javac_path):
                paths['javac'] = javac_path
                logger.info(f"Found Javac at: {javac_path}")
                break
        
        # Log final paths
        logger.info(f"Final Java paths - Java: {paths['java']}, Javac: {paths['javac']}")
    ...
    def execute_code(self, code: str, language: str, input_data: str, 
        ...
        if language == "java" and (not self.java_paths['java'] or not self.java_paths['javac']):
            logger.error(f"Java not available - Java: {self.java_paths['java']}, Javac: {self.java_paths['javac']}")
            logger.error(f"JAVA_HOME: {os.environ.get('JAVA_HOME')}")
            logger.error(f"PATH: {os.environ.get('PATH')}")
            return {
                "verdict": VerdictType.CE,
                "output": "",
                "error": f"Java compiler not available. Please ensure Java is properly installed.\nJava: {self.java_paths['java']}\nJavac: {self.java_paths['javac']}\nJAVA_HOME: {os.environ.get('JAVA_HOME')}",
                "execution_time_ms": 0,
                "memory_used_kb": 0
            }
        ...
        try:
            ...
            if config["compile_command"]:
                ...
                compile_cmd = [
                    arg.format(
                        ...
                        classname=class_name if language == "java" else "solution",
                        javac_path=self.java_paths['javac'] or "javac"
                    )
                    for arg in config["compile_command"]
                ]
            ...
            run_cmd = [
                arg.format(
                    ...
                    executable="solution", 
                    classname=class_name if language == "java" else "solution",
                    java_path=self.java_paths['java'] or "java"
                )
                for arg in config["run_command"]
            ]
