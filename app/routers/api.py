...
from app.models.assessment import AssessmentCandidate
from app.models.proctoring import ProctoringEvent, ProctoringEventType
from app.routers.candidate import get_current_candidate_from_cookie   # ✅ fixed import
from app.services.code_executor import execute_code_async
...
async def execute_code(
    request_data: CodeExecutionRequest,
    current_candidate: AssessmentCandidate = Depends(get_current_candidate_from_cookie),   # ✅ fixed
    db: Session = Depends(get_db)
):
    try:
        # Verify question is part of assessment
...
async def submit_mcq_answer(
    mcq_data: MCQSubmissionRequest,
    current_candidate: AssessmentCandidate = Depends(get_current_candidate_from_cookie),   # ✅ fixed
    db: Session = Depends(get_db)
):
    try:
        # Verify question is part of assessment and is MCQ
        question = None
        for aq in current_candidate.assessment.assessment_questions:
            if aq.question_id == mcq_data.question_id:
        ...
        if question.question_type.value != 'mcq':
            raise HTTPException(status_code=400, detail="Question is not MCQ type")
        
        # Calculate score
        score = 0.0
        is_correct = False
        
        if question.correct_answer and question.correct_answer.lower() == mcq_data.answer.lower():
            score = question.max_score
            is_correct = True
        
        from app.models.submission import VerdictType   # ✅ ensure VerdictType imported
        ...
        submission = Submission(
            ...
            assessment_id=current_candidate.assessment_id,
            code=f"Selected Answer: {mcq_data.answer}",
            language="mcq",
            is_final_submission=True,
            status=SubmissionStatus.COMPLETED,
            overall_verdict=VerdictType.OK if is_correct else VerdictType.WA,
            total_score=score
        )
        ...
        calculate_assessment_score(current_candidate, db)
        ...
        raise HTTPException(status_code=500, detail="MCQ submission failed")
